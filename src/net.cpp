...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    RenameThread("BitCurrency-ext-ip");
...
static const char *strDNSSeed[][2] = {		//seednodes
    {"198.105.125.83", "198.105.125.83"},
    {"198.105.122.107", "198.105.122.107"}
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("BitCurrency-dnsseed");
...
void ThreadDumpAddress(void* parg)
{
    ...
    RenameThread("BitCurrency-adrdump");
...
void ThreadOpenConnections(void* parg)
{
    ...
    RenameThread("BitCurrency-opencon");
...
void ThreadMessageHandler(void* parg)
{
    ...
    RenameThread("BitCurrency-msghand");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BitCurrency is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(void* parg)
{
    ...
    RenameThread("BitCurrency-start");
